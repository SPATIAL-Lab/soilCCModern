...
sm_forward_opt_wq = function(Pa, Tma, PfPCQ, TmPCQ_min_a, Ra, TmWQ_min_a, DQ){
...
sm_forward_opt_dq = function(Pa, Tma, PfPCQ, TmPCQ_min_a, Ra, TmWQ_min_a, DQ){
...
wq_pred_opt = data.frame(d13C=numeric(0), d18O=numeric(0), d13C_sd=numeric(0), d18O_sd=numeric(0))
for(i in 1: nrow(sites)){
  wq_pred_opt[i,] = sm_forward_opt_wq(sites$Pa[i], sites$Tma[i], sites$PfWQ[i], sites$TmWQ_min_a[i], sites$Ra[i], sites$TmWQ_min_a[i], 3)
  
}
wq_pred_opt$Site = sites$Site

dq_pred_opt = data.frame(d13C=numeric(0), d18O=numeric(0), d13C_sd=numeric(0), d18O_sd=numeric(0))
for(i in 1: nrow(sites)){
  dq_pred_opt[i,] = sm_forward_opt_dq(sites$Pa[i], sites$Tma[i], sites$PfDQ[i], sites$TmDQ_min_a[i], sites$Ra[i], sites$TmWQ_min_a[i], sites$DQ[i])
  
}
dq_pred_opt$Site = sites$Site


wq.comp.opt = merge.data.frame(wq_pred_opt, sites, by.x = "Site", by.y = "Site", all.x=TRUE)
dq.comp.opt = merge.data.frame(dq_pred_opt, sites, by.x = "Site", by.y = "Site", all.x=TRUE)

wq.comp.opt = merge.data.frame(wq.comp.opt, data.aves, by.x = "Site", by.y = "Site", all.x=TRUE)
dq.comp.opt = merge.data.frame(dq.comp.opt, data.aves, by.x = "Site", by.y = "Site", all.x=TRUE)

#add it to the predictions and plot

c = ceiling((wq.comp.opt$Pa / 1000) * 5)

pal_map = brewer.pal(5, "Blues")

d13C_al_opt <- c(min(c((wq.comp.opt$d13C - wq.comp.opt$d13C_sd), (wq.comp.opt$d13C.measured - C_site_sd), (dq.comp.opt$d13C - dq.comp.opt$d13C_sd), (dq.comp.opt$d13C.measured - C_site_sd))), max(c((wq.comp.opt$d13C + wq.comp.opt$d13C_sd), (wq.comp.opt$d13C.measured + C_site_sd), (dq.comp.opt$d13C + dq.comp.opt$d13C_sd), (dq.comp.opt$d13C.measured + C_site_sd))))
d18O_al_opt <- c(min(c((wq.comp.opt$d18O - wq.comp.opt$d18O_sd), (wq.comp.opt$d18O.measured - O_site_sd), (dq.comp.opt$d18O - dq.comp.opt$d18O_sd), (dq.comp.opt$d18O.measured - O_site_sd))), max(c((wq.comp.opt$d18O + wq.comp.opt$d18O_sd), (wq.comp.opt$d18O.measured + O_site_sd), (dq.comp.opt$d18O + dq.comp.opt$d18O_sd), (dq.comp.opt$d18O.measured + O_site_sd))))

jpeg("Optimized.jpg", units="in", res=300, width=8.5, height=7.5)

layout(matrix(c(1,2,3,4), 2, 2, byrow=TRUE), heights=c(1,1.1,1,1.1), widths=c(1,1.05,1,1.05))
par(mar=c(2,5,1,0))
plot(wq.comp.opt$d13C.measured, wq.comp.opt$d13C, pch=16, col=pal_map[c], xlim=d13C_al_opt, ylim=d13C_al_opt, main="", cex = 1.25,
     xlab="", cex.axis=1.25, cex.lab=1.25,
     ylab=expression(paste("Predicted ",delta^{13}, "C"[carbonate], "(\u2030)")),
     xaxt='n')
arrows(wq.comp.opt$d13C.measured, wq.comp.opt$d13C - wq.comp.opt$d13C_sd,wq.comp.opt$d13C.measured, wq.comp.opt$d13C + wq.comp.opt$d13C_sd, angle=90, length=0.1, code = 3, col=alpha("black", 0.2))
arrows(wq.comp.opt$d13C.measured - C_site_sd, wq.comp.opt$d13C, wq.comp.opt$d13C.measured + C_site_sd, wq.comp.opt$d13C, angle=90, length=0.1, code = 3, col=alpha("black", 0.2))
abline(0,1)
points(wq.comp.opt$d13C.measured, wq.comp.opt$d13C, pch=1, cex=1.3)
text(-15,10,"a", cex=1.5)


par(mar=c(2,5,1,1))
plot(wq.comp.opt$d18O.measured, wq.comp.opt$d18O, pch=16, col=pal_map[c], xlim=d18O_al_opt, ylim=d18O_al_opt,main="",cex = 1.25,
     xlab="",  cex.axis=1.25, cex.lab=1.25,
     ylab=expression(paste("Predicted ",delta^{18}, "O"[carbonate], "(\u2030)")),
     xaxt='n')
arrows(wq.comp.opt$d18O.measured, wq.comp.opt$d18O - wq.comp.opt$d18O_sd,wq.comp.opt$d18O.measured, wq.comp.opt$d18O + wq.comp.opt$d18O_sd, angle=90, length=0.1, code = 3, col=alpha("black", 0.2))
arrows(wq.comp.opt$d18O.measured - O_site_sd, wq.comp.opt$d18O, wq.comp.opt$d18O.measured + O_site_sd, wq.comp.opt$d18O,  angle=90, length=0.1, code = 3, col=alpha("black", 0.2))
abline(0,1)
points(wq.comp.opt$d18O.measured, wq.comp.opt$d18O, pch=1, cex=1.3)
text(-26,9,"b", cex=1.5)

par(mar=c(4,5,0,0))
plot(dq.comp.opt$d13C.measured, dq.comp.opt$d13C, pch=16, col=pal_map[c], xlim=d13C_al_opt, ylim=d13C_al_opt, main="",cex = 1.25,
     xlab=expression(paste("Observed ",delta^{13}, "C"[carbonate], "(\u2030)")),  cex.axis=1.25, cex.lab=1.25,
     ylab=expression(paste("Predicted ",delta^{13}, "C"[carbonate], "(\u2030)")))
arrows(dq.comp.opt$d13C.measured, dq.comp.opt$d13C - dq.comp.opt$d13C_sd,dq.comp.opt$d13C.measured, dq.comp.opt$d13C + dq.comp.opt$d13C_sd, angle=90, length=0.1, code = 3, col=alpha("black", 0.2))
arrows(dq.comp.opt$d13C.measured - C_site_sd, dq.comp.opt$d13C, dq.comp.opt$d13C.measured + C_site_sd, dq.comp.opt$d13C, angle=90, length=0.1, code = 3, col=alpha("black", 0.2))
abline(0,1)
points(dq.comp.opt$d13C.measured, dq.comp.opt$d13C, pch=1, cex=1.3)
text(-15,10,"c", cex=1.5)

par(mar=c(4,5,0,1))
plot(dq.comp.opt$d18O.measured, dq.comp.opt$d18O, pch=16, col=pal_map[c], xlim=d18O_al_opt, ylim=d18O_al_opt, main="",cex = 1.25,
     xlab=expression(paste("Observed ",delta^{18}, "O"[carbonate], "(\u2030)")),  cex.axis=1.25, cex.lab=1.25,
     ylab=expression(paste("Predicted ",delta^{18}, "O"[carbonate], "(\u2030)")))
arrows(dq.comp.opt$d18O.measured, dq.comp.opt$d18O - dq.comp.opt$d18O_sd,dq.comp.opt$d18O.measured, dq.comp.opt$d18O + dq.comp.opt$d18O_sd, angle=90, length=0.1, code = 3, col=alpha("black", 0.2))
arrows(dq.comp.opt$d18O.measured - O_site_sd, dq.comp.opt$d18O, dq.comp.opt$d18O.measured + O_site_sd, dq.comp.opt$d18O,  angle=90, length=0.1, code = 3, col=alpha("black", 0.2))
abline(0,1)
points(dq.comp.opt$d18O.measured, dq.comp.opt$d18O, pch=1, cex=1.3)
text(-26,9,"d", cex=1.5)
...
## Basic stats
opt_lm_warm_C <- lm(wq.comp.opt$d13C ~ wq.comp.opt$d13C.measured)
opt_lm_dry_C <- lm(dq.comp.opt$d13C ~ dq.comp.opt$d13C.measured)
opt_lm_warm_O <- lm(wq.comp.opt$d18O ~ wq.comp.opt$d18O.measured)
opt_lm_dry_O <- lm(dq.comp.opt$d18O ~ dq.comp.opt$d18O.measured)

summary(opt_lm_warm_C)
summary(opt_lm_dry_C)
summary(opt_lm_warm_O)
...
opt_warm_C_rmse <- rmse(wq.comp.opt$d13C.measured - wq.comp.opt$d13C)
opt_dry_C_rmse <- rmse(dq.comp.opt$d13C.measured - dq.comp.opt$d13C)
opt_warm_O_rmse <- rmse(wq.comp.opt$d18O.measured - wq.comp.opt$d18O)
opt_dry_O_rmse <- rmse(dq.comp.opt$d18O.measured - dq.comp.opt$d18O)
...
diff_warm_C_rmse <- opt_warm_C_rmse - pre_warm_C_rmse
diff_dry_C_rmse <- opt_dry_C_rmse - pre_dry_C_rmse
diff_warm_O_rmse <- opt_warm_O_rmse - pre_warm_O_rmse
diff_dry_O_rmse <- opt_dry_O_rmse - pre_dry_O_rmse

abs(diff_warm_C_rmse) / pre_warm_C_rmse * 100
abs(diff_dry_C_rmse) / pre_dry_C_rmse * 100
abs(diff_warm_O_rmse) / pre_warm_O_rmse * 100

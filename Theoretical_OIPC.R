...
sm_forward_evap.OIPC = function(Pa, Tma, PfPCQ, TmPCQ_min_a, Ra, TmWQ_min_a, DQ, O_1, O_2, O_3, O_4){
  ...
  dO_P_OOS <- ifelse(DQ == 3, O_3, ifelse(DQ == 1, O_1, ifelse(DQ == 2, O_2, O_4)))
  dO_P_PCQ <- ifelse(DQ == 3, mean(O_1, O_2, O_4), ifelse(DQ == 1, mean(O_3, O_2, O_4), ifelse(DQ == 2, mean(O_1, O_3, O_4), mean(O_1, O_2, O_3))))
...
# Make sure seasons are properly defined in S hemisphere, 1 is winter, 2 is spring, 3 is summer, 4 is fall, just like DQ

wq_pred.OIPC = data.frame(d13C=numeric(0), d18O=numeric(0), d13C_sd=numeric(0), d18O_sd=numeric(0))
for(i in 1: nrow(sites_OIPC)){
  wq_pred.OIPC[i,] = sm_forward_evap.OIPC(sites_OIPC$Pa[i], sites_OIPC$Tma[i], sites_OIPC$PfWQ[i], sites_OIPC$TmWQ_min_a[i], sites_OIPC$Ra[i], sites_OIPC$TmWQ_min_a[i], 3, sites_OIPC$d18O_OIPC_1[i], sites_OIPC$d18O_OIPC_2[i], sites_OIPC$d18O_OIPC_3[i], sites_OIPC$d18O_OIPC_4[i])
  
}
wq_pred.OIPC$Site = sites_OIPC$Site

dq_pred.OIPC = data.frame(d13C=numeric(0), d18O=numeric(0), d13C_sd=numeric(0), d18O_sd=numeric(0))
for(i in 1: nrow(sites_OIPC)){
  dq_pred.OIPC[i,] = sm_forward_evap.OIPC(sites_OIPC$Pa[i], sites_OIPC$Tma[i], sites_OIPC$PfDQ[i], sites_OIPC$TmDQ_min_a[i], sites_OIPC$Ra[i], sites_OIPC$TmWQ_min_a[i], sites_OIPC$DQ[i], sites_OIPC$d18O_OIPC_1[i], sites_OIPC$d18O_OIPC_2[i], sites_OIPC$d18O_OIPC_3[i], sites_OIPC$d18O_OIPC_4[i])
  
}
dq_pred.OIPC$Site = sites_OIPC$Site

## Plots of measured vs. predicted d13C and d18O values with Pa colors
wq_comp_pre.OIPC = merge.data.frame(wq_pred.OIPC, sites_OIPC, by.x = "Site", by.y = "Site", all.x=TRUE)
dq_comp_pre.OIPC = merge.data.frame(dq_pred.OIPC, sites_OIPC, by.x = "Site", by.y = "Site", all.x=TRUE)

wq_comp_pre.OIPC = merge.data.frame(wq_comp_pre.OIPC, data.aves, by.x = "Site", by.y = "Site", all.x=TRUE)
dq_comp_pre.OIPC = merge.data.frame(dq_comp_pre.OIPC, data.aves, by.x = "Site", by.y = "Site", all.x=TRUE)

# Blue scale for precipitation
c = ceiling(((wq_comp_pre.OIPC$Pa) / 1000) * 5)
pal_Pa = brewer.pal(5, "Blues")

# Axes for d13C and d18O
d13C_al.OIPC <- c(min(c((wq_comp_pre.OIPC$d13C - wq_comp_pre.OIPC$d13C_sd), (wq_comp_pre.OIPC$d13C.measured - C_site_sd), (dq_comp_pre.OIPC$d13C - dq_comp_pre.OIPC$d13C_sd), (dq_comp_pre.OIPC$d13C.measured - C_site_sd))), max(c((wq_comp_pre.OIPC$d13C + wq_comp_pre.OIPC$d13C_sd), (wq_comp_pre.OIPC$d13C.measured + C_site_sd), (dq_comp_pre.OIPC$d13C + dq_comp_pre.OIPC$d13C_sd), (dq_comp_pre.OIPC$d13C.measured + C_site_sd))))
d18O_al.OIPC <- c(min(c((wq_comp_pre.OIPC$d18O - wq_comp_pre.OIPC$d18O_sd), (wq_comp_pre.OIPC$d18O.measured - O_site_sd), (dq_comp_pre.OIPC$d18O - dq_comp_pre.OIPC$d18O_sd), (dq_comp_pre.OIPC$d18O.measured - O_site_sd))), max(c((wq_comp_pre.OIPC$d18O + wq_comp_pre.OIPC$d18O_sd), (wq_comp_pre.OIPC$d18O.measured + O_site_sd), (dq_comp_pre.OIPC$d18O + dq_comp_pre.OIPC$d18O_sd), (dq_comp_pre.OIPC$d18O.measured + O_site_sd))))

jpeg("Theoretical_OIPC.jpg", units="in", res=300, width=5, height=7.5)
  
layout(matrix(c(1,2), 2, 1, byrow=TRUE), heights=c(1,1.1), widths=c(1,1.05))
  
par(mar=c(2,5,1,1))
plot(wq_comp_pre.OIPC$d18O.measured, wq_comp_pre.OIPC$d18O, pch=16, col=pal_Pa[c], xlim=d18O_al.OIPC, ylim=d18O_al.OIPC,main="",cex = 1.25,
      xlab="",  cex.axis=1.25, cex.lab=1.25,
      ylab=expression(paste("Predicted ",delta^{18}, "O"[carbonate], "(\u2030)")),
      xaxt='n')
arrows(wq_comp_pre.OIPC$d18O.measured, wq_comp_pre.OIPC$d18O - wq_comp_pre.OIPC$d18O_sd,wq_comp_pre.OIPC$d18O.measured, wq_comp_pre.OIPC$d18O + wq_comp_pre.OIPC$d18O_sd, angle=90, length=0.1, code = 3, col=alpha("black", 0.2))
arrows(wq_comp_pre.OIPC$d18O.measured - O_site_sd, wq_comp_pre.OIPC$d18O, wq_comp_pre.OIPC$d18O.measured + O_site_sd, wq_comp_pre.OIPC$d18O,  angle=90, length=0.1, code = 3, col=alpha("black", 0.2))
abline(0,1)
points(wq_comp_pre.OIPC$d18O.measured, wq_comp_pre.OIPC$d18O, pch=1, cex=1.3)
text(-34,9,"a", cex=1.5)
  
par(mar=c(4,5,0,1))
plot(dq_comp_pre.OIPC$d18O.measured, dq_comp_pre.OIPC$d18O, pch=16, col=pal_Pa[c], xlim=d18O_al.OIPC, ylim=d18O_al.OIPC, main="",cex = 1.25,
      xlab=expression(paste("Observed ",delta^{18}, "O"[carbonate], "(\u2030)")),  cex.axis=1.25, cex.lab=1.25,
      ylab=expression(paste("Predicted ",delta^{18}, "O"[carbonate], "(\u2030)")))
arrows(dq_comp_pre.OIPC$d18O.measured, dq_comp_pre.OIPC$d18O - dq_comp_pre.OIPC$d18O_sd,dq_comp_pre.OIPC$d18O.measured, dq_comp_pre.OIPC$d18O + dq_comp_pre.OIPC$d18O_sd, angle=90, length=0.1, code = 3, col=alpha("black", 0.2))
arrows(dq_comp_pre.OIPC$d18O.measured - O_site_sd, dq_comp_pre.OIPC$d18O, dq_comp_pre.OIPC$d18O.measured + O_site_sd, dq_comp_pre.OIPC$d18O,  angle=90, length=0.1, code = 3, col=alpha("black", 0.2))
abline(0,1)
points(dq_comp_pre.OIPC$d18O.measured, dq_comp_pre.OIPC$d18O, pch=1, cex=1.3)
text(-34,9,"b", cex=1.5)
...
## Basic stats
pre_lm_warm_O.OIPC <- lm(wq_comp_pre.OIPC$d18O ~ wq_comp_pre.OIPC$d18O.measured)
pre_lm_dry_O.OIPC <- lm(dq_comp_pre.OIPC$d18O ~ dq_comp_pre.OIPC$d18O.measured)

summary(pre_lm_warm_O.OIPC)
summary(pre_lm_dry_O.OIPC)
...
pre_warm_C_rmse.OIPC <- rmse(wq_comp_pre.OIPC$d13C.measured - wq_comp_pre.OIPC$d13C)
pre_dry_C_rmse.OIPC <- rmse(dq_comp_pre.OIPC$d13C.measured - dq_comp_pre.OIPC$d13C)
pre_warm_O_rmse.OIPC <- rmse(wq_comp_pre.OIPC$d18O.measured - wq_comp_pre.OIPC$d18O)
pre_dry_O_rmse.OIPC <- rmse(dq_comp_pre.OIPC$d18O.measured - dq_comp_pre.OIPC$d18O)

pre_warm_C_rmse.OIPC
pre_dry_C_rmse.OIPC
pre_warm_O_rmse.OIPC
pre_dry_O_rmse.OIPC

sm_optimizer_r = function(Pa, Tma, PfPCQ, TmPCQ_min_a, Ra, TmWQ_min_a, DQ, rr){
...
parms_wq_C = data.frame(rr = rr, rmse = numeric(100), S_z = numeric(100))

## Warm Quarter rr optimization

for(j in 1:nrow(parms_wq_C)){
  opt = data.frame(d13C = numeric(nrow(sites)), S_z = numeric(nrow(sites)))
  for(i in 1: nrow(sites)){
    opt[i,] = sm_optimizer_r(sites$Pa[i], sites$Tma[i], sites$PfWQ[i], sites$TmWQ_min_a[i], sites$Ra[i], sites$TmWQ_min_a[i], 3, parms_wq_C$rr[j])
  }
  
  opt = data.frame(Site = sites$Site, d13C = opt$d13C, S_z = opt$S_z)
  opt = merge.data.frame(opt, data.aves, by.x = "Site", by.y = "Site", all.x=TRUE)
  
  mse = (opt$d13C - opt$d13C.measured)^2
  mse = mean(mse)
  parms_wq_C$rmse[j] = sqrt(mse)
  parms_wq_C$S_z[j] = mean(opt$S_z)
}
S_z_opt_wq = parms_wq_C[which.min(parms_wq_C$rmse),"S_z"]
rr_opt_wq = parms_wq_C[which.min(parms_wq_C$rmse),"rr"]
...
parms_dq_C = data.frame(rr = rr, rmse = numeric(100), S_z = numeric(100))

for(j in 1:nrow(parms_dq_C)){
  opt = data.frame(d13C = numeric(nrow(sites)), S_z = numeric(nrow(sites)))
  for(i in 1: nrow(sites)){
    opt[i,] = sm_optimizer_r(sites$Pa[i], sites$Tma[i], sites$PfDQ[i], sites$TmDQ_min_a[i], sites$Ra[i], sites$TmWQ_min_a[i], sites$DQ[i], parms_wq_C$rr[j])
  }
  ...
  mse = (opt$d13C - opt$d13C.measured)^2
  mse = mean(mse)
  parms_dq_C$rmse[j] = sqrt(mse)
  parms_dq_C$S_z[j] = mean(opt$S_z)
}

S_z_opt_dq = parms_dq_C[which.min(parms_dq_C$rmse),"S_z"]
rr_opt_dq = parms_dq_C[which.min(parms_dq_C$rmse),"rr"]
...
par(mar=c(4,5,1,1))
plot(parms_wq_C$rmse ~ parms_wq_C$rr, cex = 1.5, cex.axis=1.3, cex.lab=1.3, ylim=c(3,8), type="n", main = "", ylab = expression(paste("RMSE (\u2030)")), xlab = expression(paste("f"[R]," (Fraction of Estimated Respiration)")))
lines(parms_wq_C$rmse ~ parms_wq_C$rr, col="red")
lines(parms_dq_C$rmse ~ parms_dq_C$rr, col="blue")

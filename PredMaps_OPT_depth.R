...
raster_opt_wq_dC.depth = function(Pa, Tma, PfPCQ, TmPCQ_min_a, Ra){
  ...
  #Depth to carbonate constant at 50 cm
  z <- 50
  #depth in meters
  z_m <- z / 100
  ...
  R_PCQ_D <- R_PCQ_D * rr_opt_wq.depth
...
raster_opt_dq_dC.depth = function(Pa, Tma, PfPCQ, TmPCQ_min_a, Ra, TmWQ_min_a, DQ){
  ...
  #Depth to carbonate constant at 50cm
  z <- 50
  #depth in meters
  z_m <- z / 100
  ...
  #Respiration rate
  R_PCQ_D <- 1.25 * exp(0.0545 * TmPCQ) * PPCQ / (127.77 + PPCQ)  #Raich 2002, gC/m2day
  R_PCQ_D <- R_PCQ_D * rr_opt_dq.depth
  #Convert to molC/cm^3s
  R_PCQ_D = R_PCQ_D / (12.01 * 100^2)  #molC / cm2 / day
  R_PCQ_S = R_PCQ_D / (24 * 3600)  #molC/ cm2 / s
  R_PCQ_S_0 = R_PCQ_S / (L * pores) # Quade et al. (2007)
...
raster_opt_wq_dO.depth = function(Pa, Tma, PfPCQ, TmPCQ_min_a, Ra){
  ...
  dO_P_soil <- (dO_P_PCQ * PfPCQ + dO_P_OOS * (1 - spre_opt_wq.depth) * (1 - PfPCQ)) / (PfPCQ + (1 - spre_opt_wq.depth) * (1 - PfPCQ))
  ...
  R_O_soil = R_O_soil * esw_opt_wq.depth + R_O_P_soil * (1 - esw_opt_wq.depth)  
...
raster_opt_dq_dO.depth = function(Pa, Tma, PfPCQ, TmPCQ_min_a, Ra, TmWQ_min_a, DQ){
  ...
  dO_P_OOS <- -15.57 + 0.60 * TmOOS
  dO_P_PCQ <- -15.57 + 0.60 * TmPCQ
  dO_P_soil <- (dO_P_PCQ * PfPCQ + dO_P_OOS * (1 - spre_opt_dq.depth) * (1 - PfPCQ)) / (PfPCQ + (1 - spre_opt_dq.depth) * (1 - PfPCQ))
  R_O_P_soil = (dO_P_soil / 1000 + 1) * RO.vsmow
  R_O_P_PCQ = (dO_P_PCQ / 1000 + 1) * RO.vsmow
  ...
  R_O_soil <- ((R_O_surface - R_O_P_soil) * 2.71828 ^ (-z_m / z_i)) + R_O_P_soil
  #soil water is esw % evaporated fraction
  R_O_soil = R_O_soil * esw_opt_dq.depth + R_O_P_soil * (1 - esw_opt_dq.depth)  
  dO_soil <- (R_O_soil/RO.vsmow - 1) * 1000
...
raster_opt_wq_dO.depth <- Vectorize(raster_opt_wq_dO.depth)
raster_opt_dq_dO.depth <- Vectorize(raster_opt_dq_dO.depth)
raster_opt_wq_dC.depth <- Vectorize(raster_opt_wq_dC.depth)
raster_opt_dq_dC.depth <- Vectorize(raster_opt_dq_dC.depth)

setwd("C:/Users/femal/Dropbox/worldclim/")

#Use overlay to do complex raster calcs
dO_Carb_map_wq.depth <- overlay(Pa, Tma, PfWQ, TmWQ_min_a, Ra, fun=raster_opt_wq_dO.depth)
dC_Carb_map_wq.depth <- overlay(Pa, Tma, PfWQ, TmWQ_min_a, Ra, fun=raster_opt_wq_dC.depth)
dO_Carb_map_dq.depth <- overlay(Pa, Tma, PfDQ, TmDQ_min_a, Ra, TmWQ_min_a, DQ, fun=raster_opt_dq_dO.depth)
dC_Carb_map_dq.depth <- overlay(Pa, Tma, PfDQ, TmDQ_min_a, Ra, TmWQ_min_a, DQ, fun=raster_opt_dq_dC.depth)

dO_Carb_map_diff.depth <- dO_Carb_map_wq.depth - dO_Carb_map_dq.depth
dC_Carb_map_diff.depth <- dC_Carb_map_wq.depth - dC_Carb_map_dq.depth
...
d13C_R <- d13C_atmCO2 - (d13C_pf_pCO2 - W)
plant_carb_diff_map.depth <- dC_Carb_map_wq.depth - d13C_R

plot(plant_carb_diff_map.depth)

#Raster of difference between carbonate oxygen isotope values from mean annual precip and mean annual temp and our model carbonate oxygen isotope values
...
dO_diff_map.depth <- dO_Carb_map_wq.depth - dO_Carb_a

plot(dO_Carb_map_wq.depth)

plot(dO_diff_map.depth)

#Save rasters
writeRaster(dO_Carb_map_wq.depth, "dO_Carb_map_wq_depth.tif", overwrite=TRUE)
writeRaster(dC_Carb_map_wq.depth, "dC_Carb_map_wq_depth.tif", overwrite=TRUE)
writeRaster(dO_Carb_map_dq.depth, "dO_Carb_map_dq_depth.tif", overwrite=TRUE)
writeRaster(dC_Carb_map_dq.depth, "dC_Carb_map_dq_depth.tif", overwrite=TRUE)
writeRaster(dC_Carb_map_diff.depth, "dC_Carb_map_diff_depth.tif", overwrite=TRUE)
writeRaster(dO_Carb_map_diff.depth, "dO_Carb_map_diff_depth.tif", overwrite=TRUE)
writeRaster(dO_diff_map.depth, "dO_diff_map_depth.tif", overwrite=TRUE)
writeRaster(plant_carb_diff_map.depth, "plant_carb_diff_map_depth.tif", overwrite=TRUE)

#Read in rasters if they aren't already
dC_Carb_map_wq.depth <- raster("dC_Carb_map_wq_depth.tif")
dC_Carb_map_dq.depth <- raster("dC_Carb_map_dq_depth.tif")
dO_Carb_map_wq.depth <- raster("dO_Carb_map_wq_depth.tif")
dO_Carb_map_dq.depth <- raster("dO_Carb_map_dq_depth.tif")
dC_Carb_map_diff.depth <- raster("dC_Carb_map_diff_depth.tif")
dO_Carb_map_diff.depth <- raster("dO_Carb_map_diff_depth.tif")
dO_diff_map.depth <- raster("dO_diff_map_depth.tif")
plant_carb_diff_map.depth <- raster("plant_carb_diff_map_depth.tif")
...
par(mar=c(1,0,1,4))
plot(dO_Carb_map_wq.depth, xaxt = 'n', yaxt = 'n')
title("")
...
par(mar=c(1,0,1,4))
plot(dC_Carb_map_wq.depth, xaxt = 'n', yaxt = 'n')
title("")
...
par(mar=c(1,0,1,4))
plot(dO_Carb_map_diff.depth, xaxt = 'n', yaxt = 'n')
title("")
...
par(mar=c(1,0,1,4))
plot(dC_Carb_map_diff.depth, xaxt = 'n', yaxt = 'n')
title("")
...
par(mar=c(1,0,1,4))
plot(dO_diff_map.depth, xaxt = 'n', yaxt = 'n')
title("")
...
par(mar=c(1,0,1,4))
plot(plant_carb_diff_map.depth, xaxt = 'n', yaxt = 'n')
title("")
...
save.image(file="Revision_Final.RData")

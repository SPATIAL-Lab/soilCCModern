sm_forward_sens_wq_z.depth = function(Pa, Tma, PfPCQ, TmPCQ_min_a, Ra, pCO2, pores, z, DQ){
...
#depth to carbonate constant at 68.3 cm (same as 500 mm Pa and 125 mm PPCQ in other sens tests)
z = 68.3
...
for(i in 1:40){
  Pa[i]<- i*40 + 110
  Pa_sens[i,] = sm_forward_sens_wq_z.depth(Pa[i], Tma, PfWQ, TmPCQ_min_a, Ra, pCO2, pores, z, DQ)
}
SensData.depth <- cbind(Pa_sens, Pa)
...
for(i in 1:40){
  ...
  TmPCQ_min_a_sens[i,] = sm_forward_sens_wq_z.depth(Pa, Tma, PfWQ, TmPCQ_min_a[i], Ra, pCO2, pores, z, DQ)
}

SensData.depth <- cbind(SensData.depth, TmPCQ_min_a_sens, TmPCQ_min_a)
...
for(i in 1:40){
  PfWQ[i]<- i*0.01 + 0.01
  PfWQ_sens[i,] = sm_forward_sens_wq_z.depth(Pa, Tma, PfWQ[i], TmPCQ_min_a, Ra, pCO2, pores, z, DQ)
}

SensData.depth <- cbind(SensData.depth, PfWQ_sens, PfWQ)
...
for(i in 1:40){
  Tma[i]<- i*0.5 - 0.5
  Tma_sens[i,] = sm_forward_sens_wq_z.depth(Pa, Tma[i], PfWQ, TmPCQ_min_a, Ra, pCO2, pores, z, DQ)
}

SensData.depth <- cbind(SensData.depth, Tma_sens, Tma)
...
for(i in 1:40){
  pCO2[i]<- i*50 + 50
  pCO2_sens[i,] = sm_forward_sens_wq_z.depth(Pa, Tma, PfWQ, TmPCQ_min_a, Ra, pCO2[i], pores, z, DQ)
}

SensData.depth <- cbind(SensData.depth, pCO2_sens, pCO2)
...
for(i in 1:40){
  ...
  pores_sens[i,] = sm_forward_sens_wq_z.depth(Pa, Tma, PfWQ, TmPCQ_min_a, Ra, pCO2, pores[i], z, DQ)
}

SensData.depth <- cbind(SensData.depth, pores_sens, pores)
...
for(i in 1:100){
  z[i] <- i
  z_sens[i,] <- sm_forward_sens_wq_z.depth(Pa, Tma, PfWQ, TmPCQ_min_a, Ra, pCO2, pores, z[i], DQ)
}
SensData_z.depth <- cbind(z_sens, z)

# Graphing sensitivity tests. 

jpeg("SensitivityTestGraphs_depth.jpg", units="in", res=300, width = 8, height = 14.5)

layout(matrix(c(1,2,3,4,5,6,7,8), 4, 2, byrow=TRUE), widths=c(1.2,1))

par(mar=c(4,5,1,1))
plot(d13C_Tma ~ Tma, data=SensData.depth, ylim=c(-16, -2), cex=2, cex.axis = 1.5, cex.lab = 1.7, main="", lwd=2, type="l", col="black", yaxp=c(-2, -16, 7), xlab=expression(paste("Tm"[a], " (",degree,"C)")), ylab=expression(paste(delta[carbonate], ("\u2030"))), pch=16)
par(new=TRUE)
plot(d18O_Tma ~ Tma, data=SensData.depth, col="darkgray", cex=2, cex.axis = 1.3, cex.lab = 1.5, ylim=c(-16, -2), lwd=2, type="l", xlab="", ylab="", xaxt='n', yaxt='n', pch=16)
text(0,-2.2,"a", cex=1.7)

par(mar=c(4,1,1,1))
plot(d13C_TmPCQ_min_a ~ TmPCQ_min_a, data=SensData.depth, ylim=c(-16, -2), yaxp=c(-2, -16, 7),cex=1.3, cex.axis = 1.5, cex.lab = 1.7, main="", lwd=2, type="l", col="black", yaxt='n', xlab=expression(paste("Tm"[PCQ], " - Tm"[a]," (",degree,"C)")), ylab="", pch=16)
par(new=TRUE)
plot(d18O_TmPCQ_min_a ~ TmPCQ_min_a, data=SensData.depth, col="darkgray", cex=1.3, cex.axis = 1.5, cex.lab = 1.7, ylim=c(-16, -2), lwd=2, type="l", xlab="", ylab="", xaxt='n', yaxt='n', pch=16)
legend("topright", legend=c("Carbon","Oxygen"), col=c("black","darkgray"), lty=1, pch=19, cex=1.5)
text(-5,-2.2,"b", cex=1.7)

par(mar=c(4,5,1,1))
plot(d13C_Pa ~ Pa, data=SensData.depth, col="black", main="", cex=1.3, cex.axis = 1.5, cex.lab = 1.7, ylim=c(-16, -2), xlim=c(100, 1750),lwd=2, type="l", yaxp=c(-2, -16, 7), xlab=expression(paste("P"[a]," (mm)")), ylab=expression(paste(delta[carbonate], ("\u2030"))), pch=16)
par(new=TRUE)
plot(d18O_Pa ~ Pa, data=SensData.depth, col="darkgray", cex=1.3, cex.axis = 1.5, cex.lab = 1.7, ylim=c(-16, -2),  xlim=c(100, 1750),lwd=2, type="l",xlab="", ylab="", xaxt='n', yaxt='n', pch=16)
text(100,-2.2,"c", cex=1.7)

par(mar=c(4,1,1,1))
plot(d13C_PfWQ ~ PfWQ, data=SensData.depth, col="black", cex=1.3, cex.axis = 1.5, cex.lab = 1.7, main="", yaxp=c(-2, -16, 7), ylim=c(-16, -2), xlim=c(0.01, 0.42), lwd=2, type="l", xaxp=c(0,0.5,5), xlab=expression(paste("Pf"[PCQ])), ylab="", yaxt='n', pch=16)
par(new=TRUE)
plot(d18O_PfWQ ~ PfWQ, data=SensData.depth, col="darkgray", cex=1.3, cex.axis = 1.5, cex.lab = 1.7, ylim=c(-16, -2),  xlim=c(0.01, 0.42),lwd=2, type="l", xlab="", ylab="", xaxt='n', yaxt='n', pch=16)
text(0.01,-2.2,"d", cex=1.7)

par(mar=c(4,5,1,1))
plot(d13C_pores ~ pores, data=SensData.depth, col="black", cex=1.3, cex.axis = 1.5, cex.lab = 1.7, ylim=c(-16, -2), yaxp=c(-2, -16, 7), lwd=2, type="l",xlab="Soil Porosity", ylab=expression(paste(delta[carbonate], ("\u2030"))), pch=16)
par(new=TRUE)
plot(d18O_pores ~ pores, data=SensData.depth, col="darkgray", cex=1.3, cex.axis = 1.5, cex.lab = 1.7, ylim=c(-16, -2),lwd=2, type="l", xlab="", ylab="", xaxt='n', yaxt='n', pch=16)
text(0.1,-2.2,"e", cex=1.7)

par(mar=c(4,1,1,1))
plot(d13C_pCO2 ~ pCO2, data=SensData.depth, col="black",  cex=1.3, cex.axis = 1.5, cex.lab = 1.7, xlim=c(0, 2000), ylim=c(-16, -2), lwd=2, type="l", xlab=expression(paste(italic('p'),"CO"[2]," (ppm)")), yaxt='n', ylab="", pch=16)
par(new=TRUE)
plot(d18O_pCO2 ~ pCO2, data=SensData.depth, col="darkgray",  cex=1.3, cex.axis = 1.5, cex.lab = 1.7, xlim=c(0, 2000), ylim=c(-16, -2),lwd=2, type="l", xlab="", ylab="", xaxt='n', yaxt='n', pch=16)
text(0,-2.2,"f", cex=1.7)

par(mar=c(4,5,1,1))
plot(z ~ d13C_z, data=SensData_z.depth, col="black", cex=1.3, cex.axis = 1.5, cex.lab = 1.7, xlim=c(-11, 2), ylim=c(100, 0), lwd=2, type="l", xlab=expression(paste(delta[carbonate], ("\u2030"))), pch=16, ylab="z (cm)")
text(-11, 2, "g", cex=1.7)

par(mar=c(4,1,1,1))
plot(z ~ d18O_z, data=SensData_z.depth, col="darkgray",  cex=1.3, cex.axis = 1.5, cex.lab = 1.7, xlim=c(-8, 8), ylim=c(100, 0), yaxt='n', ylab = "", lwd=2, type="l", xlab=expression(paste(delta[carbonate], ("\u2030"))), pch=16)
text(-8, 2, "h", cex=1.7)

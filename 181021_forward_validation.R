...
for(i in 1: nrow(sites)){
  hq_pred[i,] = sm_forward_sens(sites$map.wc[i], sites$mat.wc[i], sites$hqp.frac[i], sites$hqt.offset[i], sites$Ra[i], 280)
...
for(i in 1: nrow(sites)){
  dq_pred[i,] = sm_forward_sens(sites$map.wc[i], sites$mat.wc[i], sites$dqp.frac[i], sites$dqt.offset[i], sites$Ra[i], 280)
...
############### Forward model function for use in sensitivity testing
sm_optimizer = function(MAP, MAT, P_seas, T_seas, pCO2, Ra, spre, esw){
